...
"""
为飞书文档添加协作者权限脚本
用法: python add_permission.py --doc-id "文档ID" --open-id "用户OpenID"
"""
...
def add_permission(doc_id: str, open_id: str, notify: bool = True) -> dict:
    """
    为飞书文档添加用户协作者权限

    Args:
        doc_id: 文档 ID (document_id)
        open_id: 用户的 Open ID
        notify: 是否通知用户
    ...
    url = f"https://open.feishu.cn/open-apis/drive/v1/permissions/{doc_id}/members/batch_create"
    ...
    params = {
        'type': 'docx',
        'need_notification': str(notify).lower()
    }
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='为飞书文档添加协作者权限')
    ...
    result = add_permission(args.doc_id, open_id, notify=not args.no_notify)
